...
![[Pool : tableau inspiré du poème d'Eluard "La terre est bleue comme une orange"](https://www.galerie-com.com/oeuvre/la-terre-est-bleue-comme-une-orange/446882/)](img/earth_orange.jpg){width=400}
...
-   **La théorie de la terre plate** : Bien que la phénomène soit difficile à évaluer, il demeure certain que dans de nombreux pays il existe une proportion importante de personnes qui estiment que la terre est plate. Certaines études suggèrent même que le phénomène aurait tendance à augmenter, notamment chez les plus jeunes : *"La philosophie de la Terre plate existe depuis le XIXe siècle, mais elle a récemment pris de l'ampleur en ligne, en particulier sur YouTube et Twitter. Les croyants publient des vidéos et des mèmes qui défendent l'idée d'une Terre plate et avancent des théories du complot pour expliquer tout ce qui montre que la planète est en fait un globe terrestre."*[source : Pappas S., "A Third of Young Millennials Are Confused About This Incontrovertible Fact", Live Science](https://www.livescience.com/62220-millennials-flat-earth-belief.html)

-   **La science de la zone critique** offre une objection de nature différente en soulignant que le Monde n'est pas une surface mais plutôt un volume compris entre deux sphères où se déroule l'ensemble de l'activité des êtres vivants, humains ou non. Ce concept de zone critique (*earth's critical zone*) a été utilisé dans la littérature anglophone en 1998 par Gail Ashley , spécialiste américaine de sédimentologie[@council2001]. Il a ensuite été introduit en français par le géochimiste français Jérôme Gaillardet [@gaillardet2023] au milieu des années 2000 puis largement popularisé par Bruno Latour [@latour2020].
...
```{r}
lambda <- pi*D1/100 -pi/2
theta <- 2*pi*D2/100 -pi
latitude <- lambda*180/pi
longitude <- theta*180/pi
coo<-data.frame(lambda,theta, latitude, longitude)
row.names(coo)<-c("A","B","C","D","E","F",
                "G","H","I","J","K","L")
kable(coo, caprion = "Coordonnées de latitude et longitude sur la sphère")
```
...
# Points
map<-st_as_sf(coo, coords = c("longitude","latitude"))
st_crs(map)<- 4326
map$code<-row.names(coo)


mf_theme("agolalight")
mf_map(poly, pch=3, col="lightyellow")
mf_graticule(poly,lty=3)
mf_map(map, col="red",pch=20, add=T, cex=1)
mf_label(map,var = "code", pos=4, col="red", cex=1)
mf_layout("Coordonnées de latitude et longitude",frame=T, scale=F, credits="", arrow=F)
...
```{r}
library(plotly)
map$Lon<-map$theta*180/pi
map$Lat<-map$lambda*180/pi
# Create map
...
p<- plot_geo(map)%>%
  add_markers(x = ~Lon,
    ...
    add_text(x = ~Lon,
            ...
            showlegend = F) %>%
  layout(geo = g,
         title = "Projection orthographique",
         width=400,
         height = 400)
...
```{r}
distsph<-st_distance(map, by_element = F)
mat<-as.numeric(distsph)
mat<-matrix(mat, ncol=12,nrow=12, byrow=T)
mat<-mat/20000000
row.names(mat)<-code
colnames(mat)<-code
kable(mat, digits=3, caption = "Distance sphérique normalisée")
...
reg<-hclust(as.dist(mat))
plot(reg,hang = -1,
     labels = row.names(coo),
     main = "Classification en distance sphérique",
     xlab = "Positions",
     ylab = "Distance sphérique")
```
...
mf_map(poly, pch=3, col="lightyellow")
mf_graticule(poly,lty=3)
mf_map(map, col=clus,pch=20, add=T, cex=1)
mf_label(map,var = "code", pos=4, col=clus, cex=1)
mf_layout("Régionalisation sur la sphère",frame=T, scale=F, credits="", arrow=F)
...
#levels(clus)<-c("red","blue","green","black")
...
p<- plot_geo(map)%>%
  add_markers(x = ~Lon,
    ...
    add_text(x = ~Lon,
            ...
            showlegend = F) %>%
  layout(geo = g,
         title = "Régionalisation sur la sphère",
         width=400,
         height = 400)
...
::: {.callout-note title="Le package sm et les méthodes de lissage non paramétriques" collapse="true"}

Le package `sm` a été créé par ses auteurs pour illustrer une famille de méthodes de lissage non paramétriques décrités dans @bowman1997. La finalité du package est décrite comme suit par ses auteurs : 
...
par(mfrow=c(2,2), mar=c(0,0,1.3,0))

mypal<-brewer.pal(n=11, "RdYlBu")
sm.sphere(lat, lon,
          sphim=TRUE, 
          kappa=10, 
          theta=180,
          phi=90, ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 10 (north view)", cex.main= 0.8)
...
sm.sphere(lat, lon,
          ...
          phi=-90, ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 10 (south view)",cex.main=0.8)
...
sm.sphere(lat, lon,
          ...
          phi=0,ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 10 (equat. view I)", cex.main= 0.8)
...
sm.sphere(lat, lon,
          ...
          phi=0,
          addpoints = TRUE)
title(main = "Kappa = 10 (equat. view II)",cex.main=0.8)
...
sm.sphere(lat, lon,
          ...
          phi=90,ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 5 (north view)", cex.main= 0.8)
...
sm.sphere(lat, lon,
          ...
          phi=-90,ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 5 (south view)",cex.main=0.8)
...
sm.sphere(lat, lon,
          ...
          phi=0,ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 5 (equat. view I)", cex.main= 0.8)
...
sm.sphere(lat, lon,
          ...
          phi=0,ngrid=500,
          addpoints = TRUE)
title(main = "Kappa = 5 (equat. view II)",cex.main=0.8)
